...
def update_Ez(Ez, Hx, Hy, mu, eps, dt, dx):
    ...
    # Right boundary (5 px)
    for i in range(5):
        Ez[1:-1, -(i + 1)] = Ez_prev[1:-1, -(i + 2)] + coef * (
            Ez[1:-1, -(i + 2)] - Ez_prev[1:-1, -(i + 1)]
        )
    ...
    for i in range(5):
        Ez[-(i + 1), 1:-1] = Ez_prev[-(i + 2), 1:-1] + coef * (
            Ez[-(i + 2), 1:-1] - Ez_prev[-(i + 1), 1:-1]
        )
...
def grid_init(rows: int, cols: int) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    return (
        ...
        np.zeros((rows - 1, cols)),
    )


def material_init(
    path: str | None, rows: int, cols: int, black_point: float = 10.0
